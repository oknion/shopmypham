...
class Deals extends AbstractProduct
{
	...
	{
		return $this->_objectManager->get('\Sm\Deals\Helper\Data');
	}
	...
	public function _getCfg($attr = null , $data = null)
	{
		...
		$defaults = [];
		$collection = $this->_scopeConfigInterface->getValue('deals');
		...
		$_configs = $this->_scopeConfigInterface->getValue('deals');
		if (empty($_configs)) return;
		$cfgs = [];

		foreach ($groups as $group) {
			$_cfgs = $this->_scopeConfigInterface->getValue('deals/'.$group.'', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
	...
	protected function _toHtml()
	{
		...
		$folder_cache = $folder_cache.'Sm/Deals/';
		...
		$Cache_Lite = new \Sm\Deals\Block\Cache\Lite($options);
		if ($use_cache){
			...
			if ($data = $Cache_Lite->get($hash)) {
				...
				$template_file = (!empty($template_file)) ? $template_file : "Sm_Deals::default.phtml";
