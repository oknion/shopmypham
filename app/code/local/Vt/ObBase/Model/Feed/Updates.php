...
class Vt_ObBase_Model_Feed_Updates extends Vt_ObBase_Model_Feed_Abstract{
    ...
    public function getFeedUrl(){
		return Vt_ObBase_Helper_Config::UPDATES_FEED_URL;
    }
	...
	public function isInteresting($item){
		...
		$isInterestedInSelfUpgrades = array_search(Vt_ObBase_Model_Source_Updates_Type::TYPE_INSTALLED_UPDATE, $types);
		...
		foreach($types as $type){
			...
			if(($type == Vt_ObBase_Model_Source_Updates_Type::TYPE_UPDATE_RELEASE) && $isInterestedInSelfUpgrades){
				foreach($exts as $ext){
