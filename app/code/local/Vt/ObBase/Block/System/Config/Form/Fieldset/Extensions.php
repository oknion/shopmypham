...
class Vt_ObBase_Block_System_Config_Form_Fieldset_Extensions
	extends Mage_Adminhtml_Block_System_Config_Form_Fieldset
{
    ...
    public function render(Varien_Data_Form_Element_Abstract $element)
    {
        ...
        foreach ($modules as $moduleName) {
        	if (strstr($moduleName,'Vt_') === false) {
        		continue;
        	}
			
			if($moduleName == 'Vt_Core' || $moduleName == 'Vt_ObBase'){
				continue;
			}
	...
	protected function _getFieldHtml($fieldset, $moduleName)
    {
			...
			if($displayNames = unserialize($displayNames)){
				if(isset($displayNames[$moduleName])){
					...
					if($this->_convertVersion($ver) < $this->_convertVersion($version)){
						$update = '<a href="'.$url.'" target="_blank"><img src="'.$this->getSkinUrl('vt/obbase/images/update.gif').'" title="'.$this->__("Update available").'"/></a>';
						$hasUpdate = 1;
		...
		if(!$hasUpdate){
			$update = '<a  target="_blank"><img src="'.$this->getSkinUrl('vt/obbase/images/ok.gif').'" title="'.$this->__("Installed").'"/></a>';
			$moduleName ="$update $moduleName";
		}
