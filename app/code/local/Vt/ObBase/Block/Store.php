 ...
 */
class Vt_ObBase_Block_Store extends Mage_Adminhtml_Block_Template
{
    ...
    protected function _getStoreData()
    {
        ...
        if ($storeResponse !== false) {
            ...
            Mage::app()->saveCache($storeResponse, Vt_ObBase_Helper_Config::STORE_RESPONSE_CACHE_KEY);
        }
        else {
            $storeResponse =  Mage::app()->loadCache(Vt_ObBase_Helper_Config::STORE_RESPONSE_CACHE_KEY);
            if (!$storeResponse) {
    ...
    protected function _getStoreConnection()
    {
        ...
        $url = rtrim(Vt_ObBase_Helper_Config::STORE_URL) . (sizeof($url) ? ("?" . implode("&", $url)) : "");
